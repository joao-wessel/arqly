  ...
  template: `
    <section class="card overflow-hidden">
      <div class="flex flex-col gap-4 border-b border-slate-200 p-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 class="text-3xl font-extrabold tracking-tight">Usuários</h2>
          <p class="mt-2 text-slate-500">Equipe com acesso ao escritório no Arqly.</p>
        </div>
            ...
            @for (user of users(); track user.id) {
              <tr class="border-t border-slate-100">
                <td class="px-6 py-4">
                  ...
                  <p class="text-xs text-slate-500">Criado em {{ user.createdAt | date:'dd/MM/yyyy' }}</p>
                </td>
                <td class="px-6 py-4 text-slate-600">{{ user.email }}</td>
                <td class="px-6 py-4 text-slate-600">{{ user.tenantAdmin ? 'Administrador' : 'Usuário' }}</td>
                <td class="px-6 py-4">
              ...
              <tr>
                <td colspan="6" class="px-6 py-12 text-center text-slate-500">Nenhum usuário cadastrado.</td>
              </tr>
            }
          </tbody>
        </table>
        ...
        @for (user of users(); track user.id) {
          <article class="rounded-2xl border border-slate-200 bg-white p-4">
            <div class="flex items-start justify-between gap-3">
              ...
              <span class="shrink-0 rounded-full bg-arqly-50 px-3 py-1 text-xs font-bold text-arqly-700">{{ user.tenantAdmin ? 'Admin' : 'Usuário' }}</span>
            </div>
          ...
          <p class="py-8 text-center text-sm text-slate-500">Nenhum usuário cadastrado.</p>
        }
      </div>
    </section>
      ...
      <div class="modal-overlay fixed inset-0 z-50 grid place-items-center p-4">
        <form class="modal-panel card w-full max-w-xl space-y-6 p-6" [formGroup]="userForm" (ngSubmit)="saveUser()">
          <div class="flex items-center justify-between">
            <h3 class="text-xl font-extrabold">{{ editingUser() ? 'Editar usuário' : 'Novo usuário' }}</h3>
          ...
          <section class="space-y-3">
            <div>
              ...
              <p class="mt-1 text-xs text-slate-500">Identificação e credenciais de acesso ao escritório.</p>
            </div>
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <label class="space-y-1 md:col-span-2">
                <span class="text-xs font-bold text-slate-500">{{ editingUser() ? 'Nova senha' : 'Senha inicial' }} @if (!editingUser()) { <span class="text-red-500">*</span> }</span>
          ...
          <section class="space-y-3 border-t border-slate-200 pt-5">
            <div>
              ...
              <p class="mt-1 text-xs text-slate-500">Controle permissões e status de autenticação.</p>
            </div>
            <div class="grid gap-3 md:grid-cols-2">
              <label class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3">
                <span>
                  ...
                  <span class="mt-1 block text-xs text-slate-500">Inativos não conseguem autenticar.</span>
                </span>
                <input class="checkbox" type="checkbox" formControlName="active">
              </label>
              <label class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3">
                <span>
                  <span class="block text-sm font-bold text-slate-800">Administrador</span>
                  <span class="mt-1 block text-xs text-slate-500">Pode gerenciar usuários do escritório.</span>
                </span>
                <input class="checkbox" type="checkbox" formControlName="tenantAdmin">
              </label>
            </div>
          </section>
      ...
      <div class="modal-overlay fixed inset-0 z-50 grid place-items-center p-4">
        <div class="modal-panel card w-full max-w-md space-y-5 p-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-xl font-extrabold">Excluir usuário</h3>
...
export class TenantUsersComponent implements OnInit {
  ...
  saveUser() {
    ...
    request.subscribe({
      ...
      error: () => this.toast.error('Não foi possível salvar', 'Verifique os dados e tente novamente.')
    });
  }

  openDeleteModal(user: TenantUser) {
