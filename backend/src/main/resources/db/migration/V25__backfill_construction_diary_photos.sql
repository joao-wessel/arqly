-- Images uploaded before diary-photo linking was introduced remain valid files.
-- Link them to their diary entries so they appear in the gallery and, when shared,
-- in the client portal as well.
with candidates as (
    select
    ...
    where file_resource.owner_type = 'CONSTRUCTION_DIARY_ENTRY'
      and file_resource.status = 'ACTIVE'
...
select
    ...
    now(),
    now(),
    candidates.diary_entry_id,
    candidates.file_resource_id,
    coalesce(existing_orders.max_display_order, 0) + candidates.row_position,
